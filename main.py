...
def preprocess_input(raw_signal, magnitude, phase):
    raw_signal_array = np.array(raw_signal)
    magnitude_array = np.array(magnitude)
    phase_array = np.array(phase)

    # Normalize Raw_Signal using StandardScaler
    scaler = StandardScaler()
    raw_signals_normalized = scaler.fit_transform(raw_signal_array.reshape(-1, 1))

    # Pad/truncate to 80 timesteps
    X_padded = pad_sequences(raw_signals_normalized, maxlen=80, padding='post', truncating='post')
    X_input = X_padded.reshape(1, 80, 1)  # Reshape for the model
...
if uploaded_file is not None:
    ...
    try:
        with zipfile.ZipFile(uploaded_file, 'r') as zip_ref:
            ...
            # Show the data preview to the user
        ...
        st.error("Failed to open the ZIP file. The file might not be a valid ZIP file. Please check the file and try again.")
